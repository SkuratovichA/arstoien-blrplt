...
resource "aws_apprunner_service" "server" {
  ...
  # VPC networking removed to save $30/month and fix SES connectivity
  # RDS is now publicly accessible (still password protected)
  ...
  depends_on = [
    ...
    # aws_apprunner_vpc_connector.server removed - VPC connector disabled
  ]
}
...
# -----------------------------------------------------------------------------
# App Runner VPC Connector - DISABLED TO SAVE $30/MONTH
# -----------------------------------------------------------------------------
# VPC Connector removed to:
# 1. Save $30/month in AWS costs
# 2. Fix email connectivity issues (App Runner can now reach SES via internet)
# 3. Simplify architecture
# RDS is now publicly accessible with password authentication for security
